...
test.describe('Dashboard Page', () => {
    ...
    test('deve fazer upload de um arquivo', async ({ page }) => {
        ...
        // Agora isso funciona (estado existe de verdade)
        await expect(page.getByTestId('status')).toHaveText('Pendente');

        await expect(page.getByTestId('status')).toHaveText('Enviando...');

        await expect(page.getByTestId('status')).toHaveText('Concluído', {
            timeout: 10000
        });
    });
