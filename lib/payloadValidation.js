...
export function validateRequiredDate(value, _fieldName) {
    if (value == null || value.toString().trim() === "") {
        throw new Error(`Campo "${_fieldName}" é obrigatório.`);
    }
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) {
        throw new Error(`Campo "${_fieldName}" deve ser uma data válida.`);
    }
    return date;
}
