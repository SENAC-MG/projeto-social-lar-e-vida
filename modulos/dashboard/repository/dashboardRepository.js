...
export async function getDashboardChartsRepository() {
  ...
  return {
    ...
    funcionarios: {
      status: [
        ["Status", "Quantidade"],
        ...funcionariosPorStatus.map((item) => [
          item.status,
          item._count.status,
        ]),
      ],
      ...
      contratacoesMes: [
        ["Mês", "Contratações"],
        ...Object.entries(contratacoesMes),
      ],

      contratacoesAno: [
        ["Ano", "Contratações"],
        ...Object.entries(contratacoesAno),
      ],
    },
  };
}